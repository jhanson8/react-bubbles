...
Tokens are used to authenticate users, among other things. JSON tokens are used to authenticate a user by issuing a string of cryptic text that can be
stored in localStorage and used throughout the log in session.

-   [ ] What steps can you take in your web apps to keep your data secure?

One can create Private routes that require a user to login and receive a token in order to navigate to certain pages and redirected to login if token not found or issued.

-   [ ] Describe how web servers work.

Web servers work by using protocols like HTTP when a request is made to a web server and that server then fetches and submits that request using HTTP. The browser then recieves the information and displays it to the user.

-   [ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

POST, GET, PUT, DELETE can be mapped to CRUD (Create, Read, Update, Delete).
